...
declare
  ...
  memory_awarded_session_count integer := 0;
  memory_session_already_awarded boolean := false;
begin
  ...
  if p_correct
     and (
       previous.user_id is null
       or not (previous.correct_count >= 2 and previous.next_due > now_ts)
     ) then
    gained := 10 * greatest(1, least(3, p_difficulty));
  end if;

  if p_activity = 'memory' and p_correct and gained > 0 then
    memory_session_id := coalesce(
      nullif(p_submitted->>'sessionId', ''),
      'legacy:' || floor(extract(epoch from now_ts) / 1800)::text
    );
    ...
    select exists (
      ...
      where ae.user_id = p_user_id
        and ae.activity = 'memory'
        and ae.correct
        and ae.xp_awarded > 0
        and ae.created_at >= now_ts - interval '24 hours'
        and coalesce(
          nullif(ae.submitted->>'sessionId', ''),
          'legacy:' || floor(extract(epoch from ae.created_at) / 1800)::text
        ) = memory_session_id
    )
    into memory_session_already_awarded;

    if not memory_session_already_awarded then
      select count(*)::integer
      into memory_awarded_session_count
      from (
      ...
      if memory_awarded_session_count >= 3 then
        gained := 0;
      end if;
    end if;
  end if;
