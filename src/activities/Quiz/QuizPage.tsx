import { ArrowLeft, ArrowRight, CheckCircle2, HelpCircle, Keyboard, PencilLine, RotateCcw, XCircle } from "lucide-react";
import { useEffect, useMemo, useRef, useState } from "react";
import { Link, useLocation, useParams } from "react-router-dom";
import { Button } from "../../components/ui/Button";
import { ProgressRing } from "../../components/ui/ProgressRing";
import { contentIndex, getDefaultDifficulty, getMcqsForScope, getScopeLabel, parseScope } from "../../content/contentIndex";
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 rounded-lg border border-line bg-white p-4 shadow-soft lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex min-w-0 flex-col gap-2 md:flex-row md:items-center md:justify-end">
          ...
          <div className="flex min-w-0 flex-wrap gap-1" aria-label={`Question ${index + 1} of ${items.length}`}>
...
function WorkingOutBox({ itemId }: { itemId: string }) {
  ...
  const drawToPoint = (canvas: HTMLCanvasElement, point: { x: number; y: number }) => {
    const context = canvas.getContext("2d");
    ...
    <div className="mt-5 rounded-lg border border-dashed border-indigo-200 bg-indigo-50/50 p-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs font-semibold text-muted">Use this space for binary, denary, hexadecimal or file-size calculations.</p>
        </div>
        <Button variant="secondary" className="min-h-9 px-3 py-2 text-xs" onClick={clearCanvas} type="button">
          <RotateCcw size={15} />
          Clear drawing
        </Button>
      </div>
