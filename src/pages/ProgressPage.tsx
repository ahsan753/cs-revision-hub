  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-2 text-sm text-muted">Mastery is earned when each item is correct at least twice and the latest attempt is correct.</p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {contentIndex.units.map((unit) => {
          ...
          return (
            <article key={unit.id} className="rounded-lg border border-line bg-white p-5 shadow-soft">
              ...
              <div className="mt-5 space-y-3">
                {unit.subtopics.map((subtopic) => {
                  ...
                  return (
                    <div key={subtopic.id} className="grid gap-2 sm:grid-cols-[1fr_auto] sm:items-center">
                      <div>
                        ...
                        <div className="mt-1 h-2 overflow-hidden rounded-full bg-slate-100">
                          <div className="h-full rounded-full" style={{ width: `${mastery.percent}%`, background: unit.accent }} />
                        </div>
                      </div>
                      <span className="text-sm font-bold text-muted">{mastery.percent}%</span>
                    </div>
                  );
                })}
              </div>
      ...
      <section className="grid gap-4 lg:grid-cols-[1fr_1fr]">
        <div className="rounded-lg border border-line bg-white p-5 shadow-soft">
          ...
          <p className="mt-1 text-sm text-muted">Items with recent misses or low accuracy.</p>
              ...
              weakSpots.map((item) => (
                <div key={item.id} className="rounded-lg bg-slate-50 p-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-xs font-extrabold text-rose-600">{item.accuracy}%</span>
                  </div>
                  <p className="mt-1 text-xs text-muted">
                    {item.attempts} attempts / latest {item.latestCorrect ? "correct" : "missed"}
                  </p>
                </div>
              ))
            ) : (
              <p className="rounded-lg bg-emerald-50 p-4 text-sm font-bold text-emerald-700">No weak spots yet. Start practising to build a review list.</p>
            )}
          </div>
        </div>
        ...
        <div className="rounded-lg border border-line bg-white p-5 shadow-soft">
          ...
          <p className="mt-1 text-sm text-muted">
            {unlockedBadges.length} unlocked from {(contentIndex.bank.badges ?? []).length} available.
          </p>
          <div className="mt-4">
...
function getWeakSpots(progress: ReturnType<typeof useProgressStore.getState>["itemProgress"]) {
  const labels = new Map<string, string>();
  for (const unit of contentIndex.units) {
    for (const item of unit.flashcards) labels.set(item.id, `${item.subtopic} ${item.term}`);
    for (const item of unit.mcqs) labels.set(item.id, `${item.subtopic} ${item.question}`);
    for (const item of unit.codeTasks ?? []) labels.set(item.id, `${item.subtopic} ${item.prompt}`);
  }
